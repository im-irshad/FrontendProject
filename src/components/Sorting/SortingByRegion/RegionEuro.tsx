...
export default function RegionEuro() {
  const { loading, sorted, countries } = useSelector(
    (state: rootState) => state.countryReducer
  );
  console.log(sorted);
  const { regEurope, favouriteCountry } = useSelector(
    (state: rootState) => state.countryReducer
  );
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(regEuro(countries));
  }, [dispatch, countries]);
  console.log(regEurope);
  return (
    <div>
      {loading ? (
        <div>loading ...</div>
